...
def test_yaml_literal_block_produces_block_scalar_fragment() -> None:
    text = "private_key: |\n  line1value\n  line2value\n"

    fragments = parse_structured_fragments(text)

    block = next((f for f in fragments if f["structure_kind"] == "yaml_block_scalar"), None)
    assert block is not None
    assert block["key"] == "private_key"
    assert "line1value" in block["value"]
    assert "line2value" in block["value"]


def test_yaml_folded_block_produces_block_scalar_fragment() -> None:
    text = "token: >\n  partone\n  parttwo\n"
    ...
    block = next((f for f in fragments if f["structure_kind"] == "yaml_block_scalar"), None)
    assert block is not None
    assert block["key"] == "token"


def test_yaml_block_scalar_value_start_offset_is_correct() -> None:
    prefix = "other_key: normal_value\n"
    block_text = "api_key: |\n  sk-blocksecretvalue1234567890\n"
    text = prefix + block_text

    fragments = parse_structured_fragments(text)

    block = next((f for f in fragments if f["structure_kind"] == "yaml_block_scalar"), None)
    assert block is not None
    # value_start should point into the block content, after the header line
    assert block["value_start"] >= len(prefix)


def test_yaml_block_scalar_ends_at_dedent() -> None:
    text = "secret: |\n  blockline\nnext_key: normal\n"

    fragments = parse_structured_fragments(text)

    block = next((f for f in fragments if f["structure_kind"] == "yaml_block_scalar"), None)
    normal = next((f for f in fragments if f.get("structure_kind") == "yaml_scalar" or f.get("key") == "next_key"), None)
    assert block is not None
    assert "blockline" in block["value"]
    # next_key should be parsed as a regular fragment, not swallowed into block
    assert any(f.get("key") == "next_key" for f in fragments)


def test_yaml_block_scalar_empty_lines_inside_block_preserved() -> None:
    text = "key: |\n  first\n\n  second\nnext: val\n"
    ...
    block = next((f for f in fragments if f["structure_kind"] == "yaml_block_scalar"), None)
    assert block is not None
    assert "first" in block["value"]
    assert "second" in block["value"]
